...
-- 9897. Find the employee with the highest salary per department.
-- Output the department name, employee's first name along with the corresponding salary.

-- higest salry per department/ group
SELECT department,
...
SELECT department,
       ...
       salary
FROM employee
WHERE (salary, department) IN (SELECT MAX(salary), department FROM employee GROUP BY department);
